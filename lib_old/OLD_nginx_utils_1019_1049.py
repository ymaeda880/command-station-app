...
def create_self_signed_cert(
    ...
    """
    openssl を非対話で実行して自己署名証明書を作成する。
    - cn だけ指定したい場合は subj=None のままでOK（/CN=<cn> の最小サブジェクトを自動生成）
    - もっと詳細に入れたい場合は subj="/C=JP/ST=Tokyo/L=Chiyoda/O=Prec/OU=Dev/CN=home.local"
    """
    cert_path.parent.mkdir(parents=True, exist_ok=True)
    key_path.parent.mkdir(parents=True, exist_ok=True)

    _subj = subj or f"/CN={cn}"
    cmd = [
